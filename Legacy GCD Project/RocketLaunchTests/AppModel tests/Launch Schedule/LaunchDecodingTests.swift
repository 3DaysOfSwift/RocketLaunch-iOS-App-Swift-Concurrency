...
final class SpaceXAPITests: XCTestCase {
    ...
    func testHTTPFailureRemainsAnError() {
        ...
        api.fetchUpcomingLaunches { result in
            if case .failure(let error) = result { XCTAssertEqual(error as? LaunchRepositoryError, .httpStatus(503)) }
            else { XCTFail("Expected HTTP failure") }
            done.fulfill()
        }
        wait(for: [done], timeout: 3)
    }
    func testOutdatedScheduleIsCachedButCannotBecomeNext() throws {
        let began = expectation(description: "began"), done = expectation(description: "done")
        let repository = ControlledRepository()
        repository.onStart = { began.fulfill() }
        ...
        feature.refresh { done.fulfill() }; wait(for: [began], timeout: 3)
        repository.completeLatest(.success(records)); wait(for: [done], timeout: 3)
        let checked = expectation(description: "snapshot")
        feature.getSnapshot { value in
            XCTAssertEqual(value.sources[0].launches, records)
            XCTAssertEqual(value.operators.first?.name, "SpaceX")
            XCTAssertNil(value.nextLaunch)
...
private final class FailureURLProtocol: URLProtocol {
