...
/// Completion-handler transport and explicit serial-queue decoding.
final class RocketLaunchAPI: LaunchRepository {
    private let decodingQueue = DispatchQueue(label: "rocketlaunch.RocketLaunchAPI.decode", qos: .userInitiated)
    ...
    func fetchUpcomingLaunches(completion: @escaping (Result<[RocketLaunch], Error>) -> Void) -> CancellationToken {
        ...
        return LaunchHTTP.fetch(session: session, request: request, queue: decodingQueue,
                                decode: Self.decodeResponse, completion: completion)
    }
